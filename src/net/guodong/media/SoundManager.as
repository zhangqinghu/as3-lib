...
//- PRIVATE & PROTECTED VARIABLES -------------------------------------------------------------------------
		...
		private var _activeSound:Dictionary;
...
//- PUBLIC & INTERNAL METHODS -----------------------------------------------------------------------------
		...
		/**
		 * 添加sound，直接添加sound对象方便从不同地方获取sound对象
		 * @param	snd    sound
		 * @param	$name  名称
		 * @return
		 */
		...
		public function addLibrarySound($linkageID:*, $name:String):Boolean
		{
			...
			var snd:Sound = new $linkageID;
			//var symbolClass:Class = getDefinitionByName($linkageID) as Class;
			//var snd:Sound = new symbolClass as Sound;
		...
		public function playSound($name:String, $volume:Number = 1, $startTime:Number = 0, $loops:int = 0):void
		{
			...
			if (!_activeSound[$name]) {
				//trace("字典里有没有",$name);
				snd.channel.addEventListener(Event.SOUND_COMPLETE, soundCompleteListener);				
				_activeSound[$name] = snd.channel;
			}
            ...
            {
                snd.channel.removeEventListener(Event.SOUND_COMPLETE, soundCompleteListener);
                delete _activeSound[$name];
            }
		}
